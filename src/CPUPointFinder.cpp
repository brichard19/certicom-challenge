...
void checked_read(std::ifstream& file, void* ptr, size_t size)
{
  ...
  void CPUPointFinder::load(const std::string& file)
  {
    ...
    if(!file.empty() && util::file_exists(file)) {
      ...
      if(!f) throw std::runtime_error("Unable to open progress file for reading");
  ...
  void CPUPointFinder::set_callback(std::function<void(const std::vector<DistinguishedPoint>&)> callback)
  {
    _callback = callback;
  }

  void CPUPointFinder::save_progress(const std::string& file)
  {
    save_progress_impl(file);
  }

  size_t CPUPointFinder::work_per_step()
  {
    return _num_points * _iters_per_step;
  }

  int CPUPointFinder::iters_per_step()
  {
    return _iters_per_step;
  }

  int CPUPointFinder::parallel_walks()
  {
    return int(_num_points);
  }
